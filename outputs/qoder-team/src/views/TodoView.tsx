...
const TODOS: TodoItem[] = [
  { id: '1', type: 'human', title: '确认 Redis 缓存策略', assignee: '雪辰', priority: 'high', status: 'todo', due: '5/27',
    background: '鉴川的 Agent 设计了缓存方案：TTL=10min, 命中率约 85%。需产品确认是否符合业务需求。',
    materials: ['缓存策略文档', '性能测试报告'], actions: ['审阅方案', '确认 TTL', '确认降级策略'] },
  { id: '2', type: 'human', title: '审阅推荐卡片交互设计', assignee: '雪辰', priority: 'medium', status: 'todo', due: '5/28',
    background: '王辰的 Agent 已生成组件初版，需确认交互细节和视觉规范。',
    materials: ['RecommendCard.tsx', 'Figma 设计稿'], actions: ['检查渲染效果', '确认封面尺寸', '验收文案展示'] },
  { id: '3', type: 'human', title: '确认灰度发布计划', assignee: '鉴川', priority: 'high', status: 'todo', due: '5/28',
    background: '广苓的 Agent 打包了 v2.1 模型，P99=118ms，需工程确认灰度方案和回滚机制。',
    materials: ['模型性能报告', '灰度方案', '回滚手册'], actions: ['确认流量比例', '准备回滚预案'] },
  { id: '4', type: 'human', title: '更新 PRD 验收标准', assignee: '雪辰', priority: 'medium', status: 'done', due: '5/24',
    background: '根据多 Agent 讨论结果更新 PRD 验收标准。', materials: ['PRD v2.2'], actions: ['更新标准', '同步团队'] },
  { id: '5', type: 'agent', title: '推荐卡片组件开发', agent: '🎨 王辰的 Agent', agentIcon: '🎨', assignee: '王辰', priority: 'high', status: 'in-progress', due: '5/26', progress: '组件初版已生成，优化虚拟列表中' },
  { id: '6', type: 'agent', title: '接口文档 + SDK 生成', agent: '⚙️ 鉴川的 Agent', agentIcon: '⚙️', assignee: '鉴川', priority: 'high', status: 'todo', due: '5/27', progress: '接口已设计，待生成 SDK' },
  { id: '7', type: 'agent', title: '模型 v2.1 上线灰度', agent: '🧠 广苓的 Agent', agentIcon: '🧠', assignee: '广苓', priority: 'medium', status: 'todo', due: '5/28' },
  { id: '8', type: 'agent', title: '前端性能测试报告', agent: '🎨 王辰的 Agent', agentIcon: '🎨', assignee: '王辰', priority: 'low', status: 'todo', due: '5/30' },
]
...
export default function TodoView() {
  ...
  const filtered = TODOS.filter(t => {
    if (seg === 'mine') return t.assignee === '雪辰' && t.status !== 'done'
  ...
  const mineCount = TODOS.filter(t => t.assignee === '雪辰' && t.status !== 'done').length
